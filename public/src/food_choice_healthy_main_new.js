...
//healthy is always on either the left/right for the entire experiment (both phases)
//when the subject switch stores, either the healthy foods are generally better or the unhealthy foods are generally better 
//this order is randomly selected
//in the pair maker, the more highly rated food is on the first (be it healthy or unhealthy)
//the plugin checks the healthy side and puts the healthy food on that side
...
const npractice = 3; //3
const n_healthy = 40;
const n_unhealthy = 40;

const req_move_ratings = true; //true
...
function arrayToList_Healthy(array) {
  ...
  for (let i = 0; i <= array.length-1; i++) {
      list.push({stimulus: array[i], food_type: "healthy", name: healthy_names[i].name});
  }
  return list;
}
...
function arrayToList_Unhealthy(array) {
  let list =[];
  for (let i = 0; i <= array.length-1; i++) {
    list.push({stimulus: array[i], food_type: "unhealthy", name: unhealthy_names[i].name});
  }
  return list;
}
...
var get_pairs = function (array1, array2) {
  ...
  for(var i = 0; i < array1.length; i++)
  {
       ...
       list_better = list.filter(function(element){ return element.rating_diff > 0; });
       list_worse = list.filter(function(element){ return element.rating_diff < 0; });
...
console.log(healthy_stimuli_list);
console.log(unhealthy_stimuli_list);

/***********************/
/******** Survey *******/
/***********************/
//var start_exp_survey = {
//  type: 'survey-text',
//  questions: [
...
var instructions_Welcome = {
  ...
  "At the end of study you may actually receive a shipment of food based on one of your choices.  The computer randomly selects 1 in 10 participants to receive food.  If you are selected to receive food, you will be asked to provide a shipping address.<br/><br/>"+
  "Also, to receive payment for the study, you will need to provide your Venmo, PayPal, or Zelle information.<br/><br/>"+
  "Please press the spacebar to continue."],
...
var instructions_Ratings = {
  ...
  "Rate each food on a scale from 0 to 10.<br/>"+
  ...
  "Use the mouse to click on the scale to indicate your rating, then press the Continue button.<br/><br/>"+
...
var ratings = {
  timeline: [
    {
      type: "rating-scale-continue-food",
      stimulus: () => foods_to_rate[rating_order[rating_counter]].stimulus,
      stimulus_name: () => foods_to_rate[rating_order[rating_counter]].name,
      ...
      require_movement: req_move_ratings,
      ...
      on_finish: (data) => {
        ...
        if (negative_rating_counter_healthy > 18 || negative_rating_counter_unhealthy > 18){
          ratings_good = 0;
          //closeFullscreen();
...
var instructions_trials = {
  data:{
    screen_id: "instructions_trials"
  },
  type: "instructions",
  pages: ["In this part of the study, you will see two foods on the screen, one which is considered healthy and one which is considered unhealthy. <br/><br/>" + 
  "You have to choose which food you would prefer to eat. <br/><br/>"+
  "To select the left food, press the <b><font color='green'>F</font></b> key.<br/>"+
  "To select the right food, press the <b><font color='green'>J</font></b> key.<br/><br/>"+
  "After each choice, stare at the white cross at the center of the screen.<br/><br/>"+
  "When you are ready, press the spacebar to begin with a couple of practice rounds.",
  ],
  allow_backward: false, 
  key_forward: 'spacebar',
  on_finish:  function() {
    healthy_better_trials = get_pairs(healthy_good, unhealthy_good); //the better food is on the first
    unhealthy_better_trials = get_pairs(unhealthy_good, healthy_good);  //the better food is first
    //healthy_first = initial_pref(healthy_good, unhealthy_good);
    //console.log(healthy_first);
    if(healthy_first==1){
      trials_phase1 = healthy_better_trials; //healthy = 1; unhealthy = 2
      trials_phase2 = unhealthy_better_trials; //unhealthy = 1; healthy = 2
    } else {
      trials_phase1 = unhealthy_better_trials; //unhealthy = 1; healthy = 2
      trials_phase2 = healthy_better_trials; //healthy = 1; unhealthy = 2
    };
...
var practice_choices = {
  timeline: [
    fixation,
    {
      ...
      timing_response: 0,
      healthy_side:  healthy_side,
      better_type: "unhealthy", //this is totally arbritrary
...
if(healthy_side==1){
  var instructions_side = ["The healthy food will always be on the left, and the unhealthy food will always be on the right.<br/><br/>"];
...
var instructions_phase1 = {
  ...
  "You are in a virtual shop where you are choosing between the two foods presented to you.  "+  
  [instructions_shop1]+
  [instructions_side]+
  "These are real decisions.  Again, at the end of study you may actually receive a shipment of food based on one of your choices.<br/></br>"+
...
var break_node = {
  ...
  conditional_function: function(){
      if(choice_count_phase1 == ntrials/2 || choice_count_phase2 == ntrials/2){
          return true;
      } else {
          return false;
      }
...
if(healthy_first==1){
  var instructions_shop2 = ["But in this shop, the unhealthy foods are generally better than the healthy foods.<br/><br/>"];
}else if(healthy_first==0){
  var instructions_shop2 = ["But in this shop, the healthy foods are generally better than the unhealthy foods.<br/><br/>"];
}
...
var instructions_phase2 = {
  ...
  type: "instructions",
  pages: ["Now, you are in a new virtual shop.  Again, you are choosing between the two foods on the screen.  "+  
  [instructions_shop2]+
  [instructions_side]+
  "Please press the spacebar to continue to the choices."
  ],
...
var choice_phase2 = {
  timeline: [
    ...
    fixation,
    {
      ...
      timing_response: 0,
      healthy_side:  healthy_side,
      better_type: function(){ //phase 2 better type
        if(healthy_first == 1){
          return "unhealthy";
        }else if(healthy_first == 0){
          return "healthy";
...
console.log(get_food_random_chance);
...
var endofExp = function (survey_code) {
  if (get_food_random_chance == 10 && ratings_good == 1){
    reward_trial.reward_chosen = 1;
    var trials = jsPsych.data.get().filterCustom(function (trial) {
      return trial.choice_type == "phase1" || trial.choice_type == "phase2";
    })

    selectedtrialindex = getRandomInt(0, trials.count() - 1);
    selectedtrial = JSON.parse(trials.json())[selectedtrialindex];
    
    reward_trial.trial_number = selectedtrial.trial_number;
    reward_trial.choice_type = selectedtrial.choice_type;

    console.log(selectedtrial.trial_number);
    console.log(selectedtrial.choice_type);
    console.log(selectedtrial.stimulusR);
    console.log(selectedtrial.key_press);

    //check decision:  picked left or right?
    if (selectedtrial.key_press == 70){
      reward_trial.food_reward = selectedtrial.stimulusL;
    } else if (selectedtrial.key_press == 74){
      reward_trial.food_reward = selectedtrial.stimulusR;
    }

    console.log(reward_trial.food_reward);
  
    html = `<div> This is the food that you will receive:
     <br></br>
     <img height="300px" width="400px" src="${reward_trial.food_reward}"/>
      <br></br>
      Thank you for participating! You can now close this screen.
      </div>`;
  }

  else {
    html = `<div> Thank you for participating!  You can now close this screen. </div>`;
  }
...
var on_close_callback = function () {
  // jsPsych.data.displayData();
   jsPsych.data.addProperties({
    ...
    //worker_id: start_exp_survey_trial.worker_id
   });
...
var on_finish_callback = function () {
  // jsPsych.data.displayData();
   jsPsych.data.addProperties({
    ...
    windowHight: screen.height,
    //worker_id: start_exp_survey_trial.worker_id,
    reward_chosen: reward_trial.reward_chosen,
    reward_phase: reward_trial.choice_type,
    reward_food: reward_trial.food_reward
   });
   var data = JSON.stringify(jsPsych.data.get().values());
   $.ajax({
    type: "POST",
...
function startExperiment() {
  jsPsych.init({
    timeline: [
      ...
      choices_node,
      demographic_survey, payment_choice, payment_survey,
      address_node,
      success_check
    ],
